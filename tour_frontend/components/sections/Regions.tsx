...
export async function RegionsSection() {
  let regions: Awaited<ReturnType<typeof regionsApi.list>>["data"] = [];
  try {
    const res = await regionsApi.list({ limit: 8, status: "published" });
    regions = res.data;

    if (regions.length === 0) {
      const fallback = await regionsApi.list({ limit: 8 });
      regions = fallback.data;
    }
  } catch {
    regions = [];
  }
