...
export default function PhotosPage() {
  const hasResolvedInitialPhotoParam = useRef(false);
  ...
  const sortedPhotos = useMemo(() => {
    ...
    if (sortMode === "relevant") {
      ...
      return [...filteredPhotos]
        .map((photo) => {
          ...
          const editor = photo.editorName.toLowerCase();
          const collection = photo.collectionTitle.toLowerCase();

          let score = 0;
          if (title === query) score += 5;
          if (title.startsWith(query)) score += 3;
          if (title.includes(query)) score += 2;
          if (editor.includes(query)) score += 1;
          if (collection.includes(query)) score += 1;
        ...
        .sort((a, b) => {
          ...
          return (
            toTimestamp(b.photo.publishedAt) - toTimestamp(a.photo.publishedAt)
          );
        })
    ...
    if (sortMode === "oldest") {
      return [...filteredPhotos].sort(
        (a, b) => toTimestamp(a.publishedAt) - toTimestamp(b.publishedAt),
      );
    }
  ...
  }, [filteredPhotos, searchQuery, sortMode]);
  ...
  const selectedPhotoIndex = useMemo(() => {
    ...
    return sortedPhotos.findIndex((photo) => photo.id === selectedPhotoId);
  }, [sortedPhotos, selectedPhotoId]);
  ...
  const canLoadMore = visibleCount < sortedPhotos.length;
  const headerPreviews = photos.slice(0, 2);

  const closeDialog = useCallback(() => setSelectedPhotoId(null), []);

  const goNext = useCallback(() => {
    if (selectedPhotoIndex < 0 || sortedPhotos.length === 0) return;
    const nextIndex = (selectedPhotoIndex + 1) % sortedPhotos.length;
    setSelectedPhotoId(sortedPhotos[nextIndex]?.id ?? null);
  }, [sortedPhotos, selectedPhotoIndex]);

  const goPrev = useCallback(() => {
    if (selectedPhotoIndex < 0 || sortedPhotos.length === 0) return;
    const prevIndex =
      (selectedPhotoIndex - 1 + sortedPhotos.length) % sortedPhotos.length;
    setSelectedPhotoId(sortedPhotos[prevIndex]?.id ?? null);
  }, [sortedPhotos, selectedPhotoIndex]);
  ...
  useEffect(() => {
    if (selectedPhotoId && !sortedPhotos.some((photo) => photo.id === selectedPhotoId)) {
      setSelectedPhotoId(null);
    }
  }, [sortedPhotos, selectedPhotoId]);

  useEffect(() => {
    if (hasResolvedInitialPhotoParam.current) return;
    if (loading) return;
    hasResolvedInitialPhotoParam.current = true;
    ...
    const index = sortedPhotos.findIndex((photo) => photo.id === photoParam);
    if (index < 0) return;

    if (index + 1 > visibleCount) {
      setVisibleCount(index + 1);
    }
    setSelectedPhotoId(sortedPhotos[index].id);
  }, [sortedPhotos, loading, photos.length, visibleCount]);
  ...
  }, [searchQuery, selectedPhotoId, sortMode]);
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] pt-24">
      <div className="mx-auto max-w-[1120px] px-4 pb-12">
        ...
        <section className="mb-6 flex items-center justify-between gap-3">
          ...
          <div className="inline-flex items-center gap-2 rounded-md bg-white px-3 py-1.5">
            <label htmlFor="photo-sort" className="text-xs text-[#6B7280]">
              Sort by
            </label>
            <select
              id="photo-sort"
      ...
      {selectedPhoto ? (
        <div
          className="fixed inset-0 z-[70] bg-black/60 p-3 md:p-5 justify-center flex items-center overflow-autos"
          onClick={closeDialog}
        >
          <div
            className="mx-auto flex h-full w-full max-w-[1420px] md:h-[90dvh] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"
